...
describe("runInit (--cwd mode)", () => {
	...
	test("scaffolds parseable triggers.yaml + config.yaml (round-trip)", async () => {
		...
		const triggersParsed = parseTriggersConfig(yamlLoad(triggersRaw));
		...
		const configParsed = parseConfigFile(yamlLoad(configRaw));
	...
	test("omits defaultRole when multiple agents are registered", async () => {
		...
		expect(yamlLoad(configRaw)).toEqual({});
	...
	test("honors --default-role when the agent exists", async () => {
		...
		expect(result.exitCode).toBe(0);
		const config = yamlLoad(await readFile(join(tmp, ".warren/config.yaml"), "utf8")) as {
			defaultRole?: string;
		};
